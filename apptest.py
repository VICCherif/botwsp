...
headers = {"Authorization": "Bearer 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"}
...
def esperar_y_leer_respuesta():
    ...
    tiempo_maximo_espera = 40  # segundos
...
def generar_y_enviar_imagen(texto_respuesta, numero_destino):
    payload = {
...
def insertar_respuesta_en_bd(numero, respuesta, imagen_path):
    with sqlite3.connect(DB_FILE) as conn:
        cursor = conn.cursor()
        cursor.execute("INSERT INTO respuestas (wp, respuesta, imagen_path) VALUES (?, ?, ?)", (numero, respuesta, imagen_path))
        conn.commit()

def actualizar_registro_con_imagen(numero, imagen_path):
    with sqlite3.connect(DB_FILE) as conn:
        cursor = conn.cursor()
        cursor.execute("UPDATE respuestas SET imagen_path = ? WHERE wp = ? ORDER BY id DESC LIMIT 1", (imagen_path, numero))
        conn.commit()
        ...
        if not numero_enviado(telefono_envia):
            for template_index, template in enumerate(templates):
                if enviar_mensaje(template_index, telefono_envia):
                    ...
                    # Espera una respuesta después de cada mensaje enviado, no solo el último
                    ...
                    if respuesta:
                        ...
                        # Solo para el último template, genera y envía la imagen basada en la respuesta

                        if template_index == len(templates) - 1:
                            ms2 = messenger.send_template_message(telefono_envia, 'stero_picnic_5')
                            print(ms2)                            
                            if not generar_y_enviar_imagen(respuesta, telefono_envia):
